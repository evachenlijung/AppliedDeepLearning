...
def parse_args():
    ...
    parser.add_argument(
        "--context_file", type=str, default=None, required=True, help="A csv or a json file containing the context data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--validation_file", type=str, default=None, required=True, help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--test_file", type=str, default=None, required=True, help="A csv or a json file containing the Prediction data."
    )
    parser.add_argument(
        "--max_seq_length",
        type=int,
...
def main():
    ...
    def prepare_train_features(examples):
        ...
        contexts = [paragraphs[id] for id in examples[context_column_name]]
    ...
    # Post-processing:
    def post_processing_function(examples, features, predictions, stage):
        ...
        if stage == "eval":
            # ans = {"text": examples[answer_column_name]["text"], "answer_start": examples[answer_column_name]["start"]}
            ids = examples["id"]
            ans_texts = examples[answer_column_name]["text"]
            ans_starts = examples[answer_column_name]["start"]
            # references = [{"id": ids, "answers": {"text": ans_texts, "answer_start": ans_starts}}]
            references = []
            for id, text, start in zip(ids, ans_texts, ans_starts):
                text_list = [text] if isinstance(text, str) else text
                start_list = [start] if isinstance(start, int) else start
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        logger.info(f"***** Running Evaluation {epoch}*****")
        ...
        prediction = post_processing_function(eval_examples, eval_dataset, outputs_numpy, "eval")
        eval_metric = metric.compute(predictions=prediction.predictions, references=prediction.label_ids)
        logger.info(f"Evaluation metrics: {eval_metric}")
        # em_value.append(eval_metric)
        em_value.append(eval_metric["exact_match"])
    ...
    loops = list(range(1,args.num_train_epochs+1, 1))
    plt.figure(figsize=(8,4))
    plt.plot(loops, train_loss, label="Training Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Training Loss")
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True)) 
    plt.legend()
    filepath = os.path.join(args.output_dir, f"{timestamp}_training_loss.png")
    plt.savefig(filepath, bbox_inches="tight")

    plt.figure(figsize=(8,4))
    plt.plot(loops, em_value, label="Exact Match metric value")
    plt.xlabel("Epoch")
    plt.ylabel("Exact Match")
    plt.title("Exact Match metric value")
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True)) 
    plt.legend()
    filepath = os.path.join(args.output_dir, f"{timestamp}_exact_match_metric_value.png")
    plt.savefig(filepath, bbox_inches="tight")
    ...
    if args.do_predict:
        ...
        outputs_numpy = (start_logits_concat, end_logits_concat)
        prediction = post_processing_function(predict_examples, predict_dataset, outputs_numpy, "test")

        output_csv_file = os.path.join(args.output_dir, "prediction.csv")
